...
E('results').onclick=async function(){var r=await raw('/api/student/latest-report');E('detail').innerHTML='<div class="panel"><h3>'+(r.term?esc(r.term.name):'Latest Report')+'</h3>'+table(r.subjects,[{key:'subject_name',label:'Subject'},{key:'percentage',label:'Average %'},{key:'grade'},{key:'remark'}])+'<p><b>Teacher comment:</b> '+esc(r.comments&&r.comments.class_teacher_comment||'')+'</p><p><b>Headteacher comment:</b> '+esc(r.comments&&r.comments.headteacher_comment||'')+'</p><p><b>Next term begins:</b> '+esc((r.comments&&r.comments.next_term_begins)||(r.term&&r.term.next_term_begins)||'—')+'</p></div>'};
