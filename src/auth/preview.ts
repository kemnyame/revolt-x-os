...
export async function previewRoutes(app:FastifyInstance,{db,config}:{db:Db;config:Config}){
  const createPreviewSession=async()=>{
    ...
    return {accessToken,refreshToken,expiresIn:28800,organisationId:owner.organisation_id,preview:true};
  };
